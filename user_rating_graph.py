...
edges represent if a user has favourited that anime. The edges contain weights, representing the score/rating the user
gave that anime. Recommendations are generated by first computing cosine similarity between users. Then,
...
class WeightedGraph(user_graph.Graph):
    ...
    def _score_candidate_anime(self, candidate_anime: set[str], top_users: dict[str, float]) -> dict[str, float]:
        ...
        for anime in candidate_anime:
            ...
            if total_similarity != 0 and contributors >= 2:
                ...
                anime_to_score[anime] = (round(adjusted_score, 6), contributors)
    ...
    def recommend_anime_weighted(self, inputted_ratings: dict[str, int], limit_users: int = 50, limit_anime: int = 20) \
        ...
        sorted_anime = sorted(unnormalized_scores.items(), key=lambda x: x[1][0], reverse=True)
